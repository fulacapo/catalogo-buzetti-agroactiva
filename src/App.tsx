...
import { CarouselDOM } from './components/CarouselDOM';
...
export default function App() {
  ...
  const displayProducts = useMemo(() => {
    // Sólo piezas con imagen (en el stand no queremos tarjetas vacías).
    const withImg = products.filter(p => p.imageUrl);
    if (view === 'catalog' && selectedCat)
      return withImg.filter(p => (p.categoria || p.linea) === selectedCat);
    return withImg;
